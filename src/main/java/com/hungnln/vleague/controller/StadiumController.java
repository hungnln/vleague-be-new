...
@RestController
@RequestMapping("/api/v1.0/stadiums")
...
public class StadiumController {
    ...
    ResponseEntity<ListResponseDTO> getAllStadiums(
            @RequestParam(defaultValue = "0") int pageNo,
            @RequestParam(defaultValue = "10") int pageSize
    ){
        ListResponseDTO<StadiumResponse> responseDTO = new ListResponseDTO<>();
        List<StadiumResponse> list = stadiumService.getAllStadiums(pageNo, pageSize);
    ...
    @GetMapping("/{id}")
    ResponseEntity<ResponseDTO> getStadiumById(@PathVariable UUID id){
    ...
    @PostMapping("")
    ResponseEntity<ResponseDTO> addStadium(@RequestBody @Valid StadiumCreateDTO dto) throws BindException {
        ResponseDTO<StadiumResponse> responseDTO = new ResponseDTO<>();
    ...
    @PutMapping("/{id}")
    ResponseEntity<ResponseDTO> updateStadium(@PathVariable UUID id, @RequestBody @Valid StadiumUpdateDTO dto) throws BindException{
        ResponseDTO<StadiumResponse> responseDTO = new ResponseDTO<>();
